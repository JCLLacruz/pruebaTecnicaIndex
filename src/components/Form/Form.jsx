...
const Form = ({ header, inputs }) => {
	...
	const initialSchemaRef = useRef({});
	const [inputsState, setInputsState] = useState({});
	const [hidden, setHidden] = useState(true);
	const [modalContent, setModalContent] = useState({});

	useEffect(() => {
		initialValuesRef.current = inputs.reduce((acc, input) => {
			acc[input.key] = '';
			return acc;
		}, {});

		initialSchemaRef.current = inputs.reduce((acc, input) => {
	...
	const shouldShowPlaceholder = (name) => {
		return !inputsState[name]?.isActive && !formik.values[name] && !formik.errors[name];
	  };
	...
	const formik = useFormik({
		...
		validationSchema: Yup.object().shape(initialSchemaRef.current),
		onSubmit: (values) => {
